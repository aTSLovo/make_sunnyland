...
- 获取layers数组并遍历对象
  ...
  - 如果 type : objectgroup 载入对象层
    ...
    - 关注的要点：gid查找**源图片信息**，layer数据中确认目标位置（TransformComponent）信息
      - 源图片信息如果是solid类型，则为静态障碍物，碰撞盒大小为整个图片大小
      - 源图片信息如果不是solid类型，
        - 碰撞盒大小、偏移：获取“objectgroup” -> 获取“object” -> "x", "y", "width", "height"
        - 获取其他指定自定义属性（通过模板函数），进行必要设置/添加组件（例如tag，gravity）
    - 如果没有gid，则代表自己绘制的形状（可能是碰撞盒、触发器等，未来按需处理）

- 自定义属性的获取方法，图块集中获取“tiles” -> 获取“properties”

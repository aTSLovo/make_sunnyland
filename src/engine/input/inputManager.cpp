...
void InputManager::update() {
    ...
    for(auto& [action_name, state] : action_states_) {
        if(state == ActionState::RELEASED_THIS_FRAME) {
            state = ActionState::INACTIVE;
        }
        else if(state == ActionState::PRESSED_THIS_FRAME) {
...
void InputManager::initializeMappings(const engine::core::Config* config) {
    ...
    for(const auto& [action_name, key_names] : actions_to_keyname_map_) {
        ...
        for(const auto& key_name : key_names) {
            ...
            if(scancode != SDL_SCANCODE_UNKNOWN) {      // 如果scancode有效,则将 action_name 添加到 input_to_actions_map_ 中

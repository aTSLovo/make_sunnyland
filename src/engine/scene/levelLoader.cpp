...
bool LevelLoader::loadLevel(const std::string& map_path, Scene& scene) {
    ...
    if(!file.is_open()) {
        spdlog::error("无法打开关卡文件: {}", map_path);
        return false;
    }
    ...
    if(json_data.contains("tilesets") && json_data["tilesets"].is_array()) {
        for(const auto& tileset_json : json_data["tilesets"]) {
            if(!tileset_json.contains("firstgid") || !tileset_json["firstgid"].is_number_integer()
                || !tileset_json.contains("source") || !tileset_json["source"].is_string()) {
            ...
            // spdlog::debug("levelLoader文件路径为: {}, 相对路径为: {}, 加载后的tileset路径为: {}", map_path_, tileset_json["source"].get<std::string>(), tileset_path);
    ...
    if(!json_data.contains("layers") || !json_data["layers"].is_array()) {
        spdlog::error("地图文件 '{}' 中缺少或无效的 'layers' 数组。", map_path);
        return false;
    }
    ...
    for(const auto& layer_json : json_data["layers"]) {
        ...
        if(!layer_json.value("visible", true)) {
            spdlog::info("图层 {} 不可见, 跳过加载。", json_data.value("name", "Unkown"));
            continue;
        }
...
void LevelLoader::loadImageLayer(const nlohmann::json& layer_json, Scene& scene) {
    ...
    if (image_path.empty()) {
        spdlog::error("图层 '{}' 缺少 'image' 属性。", layer_json.value("name", "Unkown"));
        return;
    }
    ...
    // 获取图层名称
    const std::string& layer_name = layer_json.value("name", "Unkown");
...
void LevelLoader::loadTileLayer(const nlohmann::json& layer_json, Scene& scene) {
    ...
    // 遍历瓦片ID列表，根据gid获取必要信息，并依次填充 TileInfo Vector
...
void LevelLoader::loadObjectLayer(const nlohmann::json& layer_json, Scene& scene) {
    ...
    for(const auto& object : objects) {
        ...
        if(gid == 0) {  // 如果gid为0 (即不存在)，则代表自己绘制的形状（可能是碰撞盒、触发器等，未来按需处理）
            ...
            auto tiles = getTileJsonByGid(gid);
            ...
            else if(auto rect = getColliderRect(tiles); rect) { // 如果非SOLID类型，检查自定义碰撞盒是否存在
                // 如果有，添加碰撞组件
            ...
            // 获取标签信息并设置
            auto tag = getTileProperty<std::string>(tiles, "tag");
            if (tag) {
            ...
            // gameScene报错--未找到玩家对象
            // if(gid == 608) {
            //     spdlog::info("***********{}***********", tag.value());
            // }

            auto gravity = getTileProperty<bool>(tiles, "gravity");
            if(gravity) {
...
std::optional<engine::utils::Rect> LevelLoader::getColliderRect(const nlohmann::json &tile_json) {
    ...
    for(const auto& object : objects) {   // 一个图片只支持一个碰撞器。如果有多个，则返回第一个不为空的
        ...
        if(rect.position.x > 0.0f || rect.position.y > 0.0f) {
            return rect;
        } 
    }
    return std::nullopt;
...
engine::component::TileType LevelLoader::getTileTypeById(const nlohmann::json& tileset_json, int local_id) {
    if(tileset_json.contains("tiles")) {
        for(auto& tile_json : tileset_json["tiles"]) {
...
engine::component::TileInfo LevelLoader::getTileInfoByGid(int gid) {
    ...
    int local_id = gid - tileset_it->first;                         // 计算瓦片在图块集中的局部ID
    ...
    // 图块集分为两种情况，需要分别考虑
    if(tileset.contains("image")) { // 这是单一图片的情况，瓦片大小相同
        // 获取图片路径
        auto texture_id = resolvePath(tileset["image"].get<std::string>(), file_path);
        // 计算瓦片在图片tileset.png网格中的坐标
...
void LevelLoader::loadTileset(const std::string& tileset_path, int first_gid) {
    ...
    // 往tileset_data_加入数据前先在json中新增一个file_path
